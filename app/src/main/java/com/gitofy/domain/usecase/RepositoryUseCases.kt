...
class DeleteRepositoryUseCase @Inject constructor(
    ...
    suspend operator fun invoke(owner: String, name: String) = repo.deleteRepository(owner, name)
}

// PRD §6-9: Update Repository — syncs a ZIP project to an existing repo.
// `sourceZip` must already exist on disk (written once, by the caller) and
// is treated as an immutable input — never re-opened as a destination.
class UpdateRepositoryUseCase @Inject constructor(
    private val syncEngine: com.gitofy.data.repository.RepositorySyncEngine
) {
    suspend operator fun invoke(
        sourceZip: java.io.File,
        ownerLogin: String,
        repoName: String,
        commitMessage: String,
        operationDir: java.io.File
