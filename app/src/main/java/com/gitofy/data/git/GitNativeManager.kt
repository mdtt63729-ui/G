...
class GitNativeManager @Inject constructor(
    ...
    suspend fun pushZipToGithub(
        ...
        try {
            workflowInjector.injectWorkflows(root.absolutePath).getOrElse {
                return@withContext Result.failure(
                    GitOFYError.GitError("Could not prepare GitHub Actions workflows: ${it.message}")
                )
            }
    ...
    /**
     * Full-project update entry point for Gito AI attachments. The ZIP is
     * validated and extracted with the same secure pipeline as the normal
     * Update Repository flow, then libgit2 replaces the remote working tree
     * in one atomic commit/push.
     */
    suspend fun syncZipToGithub(
        repoUrl: String,
        token: String,
        zipPath: String,
        branch: String,
        commitMessage: String = "Gito: update project",
        userName: String? = null,
        userEmail: String? = null,
        callback: ProgressCallback? = null
    ): Result<String> = withContext(Dispatchers.IO) {
        ...
        val extractDir = File(context.filesDir, "gitofy_ai_extract_${System.nanoTime()}")
        val extracted = zipExtractor.extractZip(zipFile, extractDir)
        if (extracted.isFailure) {
            return@withContext Result.failure(
                GitOFYError.GitError(extracted.exceptionOrNull()?.message ?: "ZIP extraction failed")
