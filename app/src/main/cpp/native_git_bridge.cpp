...
static int credentialsCallback(git_cred** out, const char* /*url*/, const char* usernameFromUrl,
    ...
    const auto* context = static_cast<const ProgressContext*>(payload);
    const char* token = context ? context->token : nullptr;
...
Java_com_gitofy_data_git_GitNativeManager_nativePushDirectoryToGithub(
    ...
    ProgressContext context{env, callback, onProgress, onStage, isCancelled, 0, 0, 0, 0, 0, tokenChars};
    std::string result;
...
Java_com_gitofy_data_git_GitNativeManager_nativeSyncDirectoryToGithub(
    ...
    do {
        ...
        cloneOptions.fetch_opts.callbacks.payload = &context;
