...
class AIGateway @Inject constructor(
    ...
    data class GatewayRequest(
        ...
        val requireAgentActions: Boolean = false,
    ...
    suspend fun processStream(
        ...
        result.onSuccess { response ->
            providerHealth.recordSuccess(response.provider, response.latencyMs)
            usageAccountant.record(
                UsageAccountant.UsageRecord(
                    provider = response.provider, model = response.model, taskType = request.taskType,
                    tokensUsed = response.tokensUsed, reasoningTokens = response.reasoningTokens, latencyMs = response.latencyMs, success = true,
                    fellBack = response.fellBack, timestamp = System.currentTimeMillis()
                )
            )
        }.onFailure { error ->
            val message = error.message.orEmpty()
            when {
                message.contains("429") -> providerHealth.markRateLimited(model.provider)
                message.contains("timeout", true) -> providerHealth.markDegraded(model.provider)
                message.contains("500") || message.contains("502") || message.contains("503") || message.contains("504") -> providerHealth.markDegraded(model.provider)
            }
            usageAccountant.record(
                UsageAccountant.UsageRecord(
                    provider = model.provider, model = model.modelId, taskType = request.taskType, tokensUsed = 0,
                    latencyMs = System.currentTimeMillis() - startTime, success = false, fellBack = false,
                    timestamp = System.currentTimeMillis(), error = error.message
                )
            )
        }
        return result
...
class AIPolicyEnforcer @Inject constructor(
    ...
    fun check(request: AIGateway.GatewayRequest): PolicyResult {
        // Check feature flag
        val flag = if (request.requireAgentActions) {
            AIFeatureFlags.Flag.AI_AGENT_ACTIONS
        } else when (request.taskType) {
...
class UsageAccountant @Inject constructor() {
    ...
    fun getSummary(): UsageSummary {
        ...
        return UsageSummary(
            ...
            totalReasoningTokens = records.sumOf { it.reasoningTokens },
    ...
    private fun summarizeList(list: List<UsageRecord>): UsageSummary {
        ...
        return UsageSummary(
            ...
            totalReasoningTokens = list.sumOf { it.reasoningTokens },
            providerBreakdown = providerCounts
        )
    }
}
