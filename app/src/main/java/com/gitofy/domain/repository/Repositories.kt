...
interface WorkflowRepository {
    ...
    // PRD §9/§30: Real log download from GitHub
    suspend fun getJobLogs(owner: String, repo: String, jobId: Long): Result<String>
    suspend fun getRunLogs(owner: String, repo: String, runId: Long): Result<String>

    // PRD §36: Get a single job by ID (for repair targeting)
    suspend fun getJob(owner: String, repo: String, jobId: Long): Result<JobSummary>

    // PRD §3.2: List runs filtered by workflow file ID
    suspend fun refreshRunsByWorkflow(owner: String, repo: String, workflowId: String): Result<List<WorkflowRunSummary>>
}
...
interface ArtifactRepository {
    ...
    /** Streaming variant used by WorkManager so callers can expose real byte progress. */
    ...
    ): Result<String> = downloadArtifact(owner, repo, artifactId, artifactName)
}
...
interface GitRepository {
    ...
    suspend fun getHeadCommitSha(directory: String): Result<String>
