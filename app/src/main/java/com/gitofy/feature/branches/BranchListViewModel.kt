...
data class BranchListUiState(
    val branches: List<BranchDetail> = emptyList(),
    val searchQuery: String = "",
    val isLoading: Boolean = false,
    val error: String? = null,
    val showDeleteConfirm: BranchDetail? = null
)
...
class BranchListViewModel @Inject constructor(
    ...
    fun load(owner: String, repo: String) {
        ...
        viewModelScope.launch {
            getBranchesUseCase(owner, repo).fold(
    ...
    fun showDeleteConfirm(branch: BranchDetail) { _uiState.update { it.copy(showDeleteConfirm = branch) } }
