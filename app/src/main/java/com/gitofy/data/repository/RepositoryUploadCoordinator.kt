...
class RepositoryUploadCoordinator @Inject constructor(
    ...
    suspend fun startUpload(
        ...
        val securityStatus = IntegritySecurityEngine.check(context)
        if (!IntegritySecurityEngine.sensitiveOperationsAllowed()) {
            throw SecurityException(
                "Gitofy security policy blocked this write operation: ${securityStatus.threats.joinToString()}"
            )
        }
        ...
        // FIX: same threading + integrity issue fixed in
        // RepositoryUpdateCoordinator — the copy ran inline on whatever
        // dispatcher the caller's viewModelScope defaults to (Main), and a
        // failed/short copy wasn't caught until the worker discovered it
        // downstream. Move the copy to Dispatchers.IO and fail fast here.
