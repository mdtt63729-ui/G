...
interface ExecJobDao {
    ...
    @Query("DELETE FROM exec_jobs WHERE ownerLogin = :owner AND repoName = :repo")
...
interface ExecJobStepDao {
    ...
    @Query("DELETE FROM exec_job_steps WHERE jobId IN (SELECT jobId FROM exec_jobs WHERE ownerLogin = :owner AND repoName = :repo)")
    suspend fun clearForRepoScopes(owner: String, repo: String)
}
...
interface ExecJobEventDao {
    ...
    @Query("DELETE FROM exec_job_events WHERE jobId IN (SELECT jobId FROM exec_jobs WHERE ownerLogin = :owner AND repoName = :repo)")
    suspend fun clearForRepoScopes(owner: String, repo: String)
}
