...
data class ReleaseAsset(
    val id: Long = 0,
    val name: String = "",
    // FIX: GitHub's actual Release Asset API field is "size" (bytes), not
    // "size_in_bytes". That wrong key meant this always deserialized to the
    // default 0, so the APK download button always showed "0 B" as the
    // real file size — this restores the correct GitHub field name.
    @SerialName("size") val sizeInBytes: Long = 0,
