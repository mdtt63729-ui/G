    ...
    fun startMonitoring(operationId: String) {
        ...
        monitorJob = viewModelScope.launch {
            operationDao.observeOperation(operationId).collectLatest { operation ->
                ...
                val speed = if (elapsedMs > 0 && operation.bytesUploaded > 0) {
                    operation.bytesUploaded.toDouble() / (elapsedMs / 1000.0)
                } else 0.0

                // PRD PHASE 13: Calculate ETA (rolling average — simple version)
                val eta = if (speed > 0 && operation.totalBytes > operation.bytesUploaded) {
