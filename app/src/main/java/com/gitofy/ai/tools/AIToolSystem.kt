...
sealed interface AITool {
    ...
    /**
     * PRD §7: True for irreversible/high-impact operations (deleting a
     * repository or branch, merging a pull request, changing visibility).
     * Destructive tools must not execute without an explicit, human-issued
     * confirmation — see [CONFIRM_TOKEN] and [requireConfirmation].
     */
...
// ---------------------------------------------------------------------------
// PRD §7: Destructive-action confirmation policy
// ---------------------------------------------------------------------------

/**
 * The exact literal a caller (the agent loop) must pass as the `confirm`
 * parameter for a destructive tool to actually execute. This is intentionally
 * a fixed, unambiguous token rather than any truthy string, so a model can't
 * satisfy it by accident (e.g. by echoing "true" as part of unrelated JSON).
 * The agent layer is responsible for only ever setting this after the human
 * user has explicitly confirmed the specific action in their own words.
 */
internal const val CONFIRM_TOKEN = "CONFIRM"

/**
 * Guards a destructive tool: returns a failed [ToolResult] explaining what
 * confirmation is required if `params["confirm"] != CONFIRM_TOKEN`, or null
 * if the caller may proceed.
 */
internal fun requireConfirmation(params: Map<String, String>, actionDescription: String): ToolResult? {
    return if (params["confirm"] != CONFIRM_TOKEN) {
        ToolResult(
            success = false,
            data = "",
            error = "CONFIRMATION_REQUIRED: $actionDescription is irreversible. " +
                "Do not proceed unless the user has explicitly confirmed this exact action in their own words. " +
                "Once confirmed, retry this tool with confirm=\"$CONFIRM_TOKEN\"."
        )
    } else null
}
 ...
 *
 * Unlike every other repository tool, this one does NOT take owner/repo —
 * the repository does not exist yet. The caller (GitHubAiAgent) skips the
 * usual "does this repo already exist / do I have write access" checks for
 * this tool and instead adopts the owner/name GitHub returns as the new
 * active repository, so a follow-up create_file call in the same
 * conversation lands in the repo that was just created.
 */
data class CreateRepositoryTool(
    val api: GitHubApiService
) : AITool {

    override val name: String = "create_repository"
    override val description: String =
        "Create a brand-new GitHub repository owned by the authenticated user. " +
            "Use this before create_file when the user asks to create/make a new repository or project."
    override val parameters: List<ToolParameter> = listOf(
        ToolParameter("name", "string", true, "Name of the new repository (no spaces or slashes)."),
        ToolParameter("description", "string", false, "Short description of the repository."),
        ToolParameter("private", "boolean", false, "true for a private repository, false (default) for public."),
        ToolParameter("auto_init", "boolean", false, "true to initialize with a README so the repo isn't empty. Defaults to true.")
    )

    override suspend fun executeSuspend(params: Map<String, String>): ToolResult {
        val missing = missingParams(params, listOf("name"))
        if (missing != null) return ToolResult(false, "", "Missing required parameter(s): ${missing.joinToString()}")
        return try {
            val resp = api.createRepository(
                com.gitofy.data.remote.dto.CreateRepoRequest(
                    name = params["name"]!!,
                    description = params["description"],
                    private = params["private"]?.toBooleanStrictOrNull() ?: false,
                    autoInit = params["auto_init"]?.toBooleanStrictOrNull() ?: true
                )
            )
            if (resp.isSuccessful) {
                val repo = resp.body()!!
                ToolResult(true, buildJsonObject {
                    put("message", "Repository created: ${repo.fullName.ifBlank { repo.name }}")
                    put("owner", repo.ownerLogin)
                    put("repo", repo.name)
                ...
                val code = resp.code()
                val hint = if (code == 422) " A repository with that name may already exist." else ""
                ToolResult(false, "", "GitHub API $code: ${resp.message()}.$hint")
            }
        } catch (e: Exception) {
            ToolResult(false, "", e.message ?: "Network error creating repository")
        }
    }
}
...
// ---------------------------------------------------------------------------
// PRD §3/§5/§7: Destructive tools — require explicit confirmation
// ---------------------------------------------------------------------------

/**
 * Permanently deletes a repository. PRD §7: irreversible, so this tool
 * refuses to run unless the caller passes `confirm="CONFIRM"`, which the
 * agent layer only sets after the human user has explicitly confirmed.
 */
data class DeleteRepositoryTool(
    val api: GitHubApiService
) : AITool {

    override val name: String = "delete_repository"
    override val description: String =
        "PERMANENTLY deletes a repository. Irreversible. Requires confirm=\"CONFIRM\", " +
            "which must only be set after the user has explicitly confirmed this exact deletion."
    override val isDestructive: Boolean = true
    override val parameters: List<ToolParameter> = listOf(
        ToolParameter("owner", "string", true, "Account owner of the repository."),
        ToolParameter("repo", "string", true, "Name of the repository to delete."),
        ToolParameter("confirm", "string", true, "Must be exactly \"CONFIRM\" — only after explicit user confirmation.")
    )
    ...
    override suspend fun executeSuspend(params: Map<String, String>): ToolResult {
        ...
        val owner = params["owner"]!!
        val repo = params["repo"]!!
        requireConfirmation(params, "Deleting repository $owner/$repo")?.let { return it }

        return try {
            val resp = api.deleteRepository(owner, repo)
            if (resp.isSuccessful) {
                ToolResult(true, buildJsonObject {
                    put("message", "Repository deleted: $owner/$repo")
                    put("owner", owner)
                    put("repo", repo)
                }.toString())
            } else {
            ...
            ToolResult(false, "", e.message ?: "Network error deleting repository")
        }
    }
}

/**
 * Deletes a branch. PRD §7: irreversible (history becomes unreachable unless
 * another ref points to it), so this requires explicit confirmation.
 */
data class DeleteBranchTool(
    val api: GitHubApiService
) : AITool {

    override val name: String = "delete_branch"
    override val description: String =
        "Permanently deletes a branch ref. Requires confirm=\"CONFIRM\", " +
            "which must only be set after the user has explicitly confirmed this exact deletion."
    override val isDestructive: Boolean = true
    override val parameters: List<ToolParameter> = listOf(
        ToolParameter("owner", "string", true, "Account owner of the repository."),
        ToolParameter("repo", "string", true, "Name of the repository."),
        ToolParameter("branch", "string", true, "Name of the branch to delete."),
        ToolParameter("confirm", "string", true, "Must be exactly \"CONFIRM\" — only after explicit user confirmation.")
    )
    ...
    override suspend fun executeSuspend(params: Map<String, String>): ToolResult {
        ...
        val repo = params["repo"]!!
        val branch = params["branch"]!!
        requireConfirmation(params, "Deleting branch $branch on $owner/$repo")?.let { return it }

        return try {
            val resp = api.deleteBranch(owner, repo, branch)
            if (resp.isSuccessful) {
                ToolResult(true, buildJsonObject {
                    put("message", "Branch deleted: $branch")
                    put("owner", owner)
                    put("repo", repo)
                    put("branch", branch)
                }.toString())
            ...
            ToolResult(false, "", e.message ?: "Network error deleting branch")
        }
    }
}

/**
 * Merges a pull request. PRD §7: merging is effectively irreversible for the
 * target branch history, so this requires explicit confirmation even though
 * a revert is technically possible afterward.
 */
data class MergePullRequestTool(
    val api: GitHubApiService
) : AITool {

    override val name: String = "merge_pull_request"
    override val description: String =
        "Merges a pull request into its base branch. Requires confirm=\"CONFIRM\", " +
            "which must only be set after the user has explicitly confirmed this exact merge."
    override val isDestructive: Boolean = true
    override val parameters: List<ToolParameter> = listOf(
        ToolParameter("owner", "string", true, "Account owner of the repository."),
        ToolParameter("repo", "string", true, "Name of the repository."),
        ToolParameter("pr_number", "string", true, "Pull request number to merge."),
        ToolParameter("merge_method", "string", false, "merge, squash, or rebase. Defaults to merge."),
        ToolParameter("confirm", "string", true, "Must be exactly \"CONFIRM\" — only after explicit user confirmation.")
    )

    override suspend fun executeSuspend(params: Map<String, String>): ToolResult {
        val missing = missingParams(params, listOf("owner", "repo", "pr_number"))
        if (missing != null) return ToolResult(false, "", "Missing required parameter(s): ${missing.joinToString()}")
        val owner = params["owner"]!!
        val repo = params["repo"]!!
        val prNumber = params["pr_number"]!!.toIntOrNull()
            ?: return ToolResult(false, "", "pr_number must be a valid integer")
        val mergeMethod = params["merge_method"]?.takeIf { it in setOf("merge", "squash", "rebase") } ?: "merge"
        requireConfirmation(params, "Merging pull request #$prNumber into $owner/$repo")?.let { return it }

        return try {
            val resp = api.mergePullRequest(
                owner, repo, prNumber,
                com.gitofy.data.remote.dto.MergeRequest(mergeMethod = mergeMethod)
            )
            if (resp.isSuccessful) {
                ToolResult(true, buildJsonObject {
                    put("message", "Pull request #$prNumber merged into $owner/$repo")
                    put("owner", owner)
                    put("repo", repo)
            ...
            ToolResult(false, "", e.message ?: "Network error merging pull request")
        }
    }
}
...
class ToolRegistry @Inject constructor(
    ...
    private val tools: Map<String, AITool> = listOf(
        ...
        // PRD §7: destructive tools — gated by requireConfirmation() internally.
        DeleteRepositoryTool(api),
        DeleteBranchTool(api),
        MergePullRequestTool(api)
    ).associateBy { it.name }

    /** Immutable, ordered view of every registered tool. */
    val all: List<AITool> get() = tools.values.toList()

    /** PRD §7: tools that are irreversible/high-impact and require explicit confirmation. */
    val destructiveToolNames: Set<String> get() = tools.values.filter { it.isDestructive }.map { it.name }.toSet()
