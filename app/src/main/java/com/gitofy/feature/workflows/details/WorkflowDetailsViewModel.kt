 ...
 * PRD §73: WorkflowRepositoryImpl emits completion/failure notifications from state transitions.
 */
            ...
            try {
                getRunUseCase(runId).collect { run ->
                    ...
                    if (isActive) {
                        ...
                        stopPolling()
                        stopTicker()
                        // Notifications are emitted by WorkflowListViewModel's single
                        // repository-level transition monitor. Keeping them out of the
                        // detail ViewModel prevents duplicate notifications when the user
                        // opens a run while it is also being observed by the list.
                        // Notification delivery is centralized in the repository so it
                        // cannot be duplicated by list/detail observers.
