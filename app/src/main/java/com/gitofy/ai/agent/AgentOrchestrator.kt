        ...
        try {
            ...
            for (task in tasks) {
                ...
                if (toolName != null && toolRegistry.contains(toolName)) {
                    // PRD §7: destructive tools must not run in AUTO/SAFE mode
                    // without an explicit confirm token — the caller has to
                    // re-invoke with executionMode = CONFIRM and confirm="CONFIRM"
                    // once the user has approved the specific action.
                    if (toolName in toolRegistry.destructiveToolNames &&
                        (executionMode != AgentExecutionMode.CONFIRM || toolParams["confirm"] != "CONFIRM")
                    ) {
                        emitEvent(
                            sessionId, AgentEventType.FAILURE,
                            "Confirmation required",
                            "$toolName is irreversible/high-impact and was not executed. Ask the user to explicitly confirm, then retry with executionMode=CONFIRM."
                        )
                        updateProgress(AgentProgressState.FAILED)
                        updateSessionStatus(sessionId, AgentSessionStatus.FAILED)
                        return sessionId
