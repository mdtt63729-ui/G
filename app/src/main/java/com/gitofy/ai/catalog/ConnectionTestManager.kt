...
class ConnectionTestManager @Inject constructor() : ProviderConnectionTester {
    ...
    override suspend fun test(apiKey: String, model: AIModelDefinition): ConnectionTestResult = withContext(Dispatchers.IO) {
        try {
    ...
    private fun testGemini(apiKey: String, model: AIModelDefinition): ConnectionTestResult {
        val endpoint = "https://generativelanguage.googleapis.com/v1beta/models/${model.id}:generateContent"
