...
fun GITOFYTonalButton(
    ...
    val hapticView = LocalView.current
    val hapticClick = { GITOFYHaptics.click(hapticView); onClick() }
    FilledTonalButton(
        onClick = hapticClick,
...
fun GITOFYCard(
    ...
    val interactionSource = remember { MutableInteractionSource() }
    val hapticView = LocalView.current
    val hapticClick: (() -> Unit)? = onClick?.let { action ->
        { GITOFYHaptics.click(hapticView); action() }
    }
...
fun PremiumSuccessCheck(
    ...
    Box(
        modifier = modifier
            ...
            .background(GITOFYStatusColors.success),
        contentAlignment = Alignment.Center
    ) {
        androidx.compose.animation.AnimatedVisibility(
            visible = shown,
