...
import com.gitofy.feature.onboarding.OnboardingScreen
import com.gitofy.feature.onboarding.IntroductionScreen
...
import com.gitofy.feature.settings.buildsettings.BuildRunSettingsScreen
...
fun GITOFYNavHost(
    ...
    val navHost: @Composable () -> Unit = {
        NavHost(
            ...
            popExitTransition = { gitofyBackExit }
                    ...
                    IntroductionScreen(
                        ...
                        onSkip = {
                            navController.navigate(Routes.HOME) {
                                popUpTo(Routes.INTRODUCTION) { inclusive = true }
                ...
                // First-launch onboarding: persistent six-step setup wizard.
                composable(Routes.ONBOARDING) {
                    OnboardingScreen(
                        onOpenGithub = { navController.navigate(Routes.SETTINGS_GITHUB) },
                        onOpenAiProviders = { navController.navigate(Routes.SETTINGS_API_PROVIDERS) },
                        // FIX: these used to have no destination at all, so
                        // tapping "Configure repository access" or "Apply
                        // personalization" did nothing visible. Repository
                        // access is governed by the same GitHub settings
                        // screen (token scope + repository defaults);
                        // personalization opens the real Appearance screen.
                        onOpenRepositorySettings = { navController.navigate(Routes.SETTINGS_GITHUB) },
                ...
                composable(Routes.EXPLORE) {
                    com.gitofy.feature.explore.ExploreScreen(
                        onRepoClick = { owner, repo -> navController.navigate(Routes.repositoryDetails(owner, repo)) },
                ...
                composable(Routes.GITO_AI) {
                    GitoAiScreen(
                        onBack = { navController.popBackStack() },
                        // FIX: this used to open the generic Settings root,
                        // leaving the user to hunt for the AI Providers
                        // section themselves after already being told "add
                        // an API key". Now it opens that screen directly.
                        onOpenProviderSettings = { navController.navigate(Routes.SETTINGS_API_PROVIDERS) }
                    )
                }
    ...
    // FIX: the area around the floating nav bar/rail/drawer used to be
    // painted with an opaque fill inside AdaptiveNavigationScaffold itself,
    // so it read as a solid strip rather than the nav bar/rail/drawer simply
    // floating over the app's own background. That opaque fill is now gone
    // from the scaffold — instead this single themed Surface sits behind
    // the whole app (scaffold + content), so anything not covered by the
    // pill/rail/drawer's own background is genuinely transparent down to
    // this real background color (correct for light/dark/dynamic color),
    // never a raw white window strip.
