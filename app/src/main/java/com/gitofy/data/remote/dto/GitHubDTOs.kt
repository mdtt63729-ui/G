...
data class Repository(
    ...
    // FIX: GitHub's repository object reports total repo size in KB via
    // "size". This wasn't modeled at all, so the source-code download
    // button had no real size to show and displayed nothing. This is the
    // repo's on-disk size (a very close proxy for the zip download size),
    // used to show a real estimated size instead of no size at all.
    @SerialName("size") val sizeInKb: Long = 0,
