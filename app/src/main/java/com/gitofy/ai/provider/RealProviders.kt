        ...
        call.enqueue(object : okhttp3.Callback {
            ...
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                response.use {
                    ...
                    if (continuation.isActive) continuation.resume(HttpResult(it.code, text), onCancellation = null)
                }
            }
        })
    }
                ...
                try {
                    response.use {
                        if (!it.isSuccessful) {
                            ...
                            if (continuation.isActive) continuation.resume(HttpResult(it.code, bodyText), onCancellation = null)
                            return
                        }
                        ...
                        if (continuation.isActive) continuation.resume(HttpResult(it.code, ""), onCancellation = null)
                    }
                } catch (t: Throwable) {
...
abstract class BaseRealProvider(
    ...
    protected fun parseUsage(root: JSONObject, fallbackText: String): Int {
        ...
        return usage?.optInt("total_tokens", -1)?.takeIf { it >= 0 }
    ...
    protected suspend fun openAiStream(
        ...
        AIProvider.GenerateResponse(content, tokensUsed = estimateTokens(content), confidence = AIConfidenceLevel.HIGH)
    }
