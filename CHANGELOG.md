...
## [4.1.1] — 2026-08-29 — UI/Motion/Notification Stability Fix Pass

### Fixed
- Replaced compact bottom navigation with a single spring-driven morphing capsule and animated active labels/icons.
- Hardened edge-to-edge system UI configuration and transparent navigation-bar handling.
- Fixed Home refresh-state declaration ordering and changed refresh rotation to run only while refreshing.
- Added consistent press-scale feedback to the shared GITOFY button component.
- Added staggered Settings category entrance motion.
- Improved upload/update current-file readability and animated file-name replacement.
- Improved success-state entrance motion and tuned the success check spring.
- Consolidated notification channel/posting implementation behind one canonical manager.
- Prevented workflow list/detail notification duplication and moved workflow transition detection to the repository layer.
- Added stable per-event notification IDs and notification permission guards.
- Replaced dead Build & Run setting click targets with working switches.
- Replaced dead About-page link targets with readable in-app documents.
