...
class GitPushWorker @AssistedInject constructor(
    ...
    private suspend fun performWork(): Result {
        ...
        try {
            ...
            // Start both workflows explicitly. Debug also runs on push, while
            // Release is workflow_dispatch/tag driven, so dispatch both here.
            for ((workflow, label) in listOf("build.yml" to "Debug", "release.yml" to "Release")) {
                val response = runCatching {
                    apiService.dispatchWorkflow(
                        createdRepo.ownerLogin.ifBlank { userLogin },
                        createdRepo.name,
                        workflow,
                        com.gitofy.data.remote.dto.DispatchWorkflowRequest(ref = branch)
                    )
                }.getOrNull()
                if (response == null || !response.isSuccessful) {
                    failStep(operationId, steps, 6, "$label workflow could not be started (HTTP ${response?.code() ?: -1})")
                    return Result.failure()
                }
            }
