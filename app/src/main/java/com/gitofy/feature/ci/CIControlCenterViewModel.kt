...
class CIControlCenterViewModel @Inject constructor(
    ...
    fun load(owner: String, repo: String) {
        ...
        viewModelScope.launch {
            getRunsUseCase(owner, repo).fold(
                onSuccess = { runs ->
                    _uiState.update {
                        it.copy(
                            running = runs.count { r -> r.status == "in_progress" },
                            queued = runs.count { r -> r.status == "queued" },
                            failed = runs.count { r -> r.conclusion == "failure" },
                            successful = runs.count { r -> r.conclusion == "success" },
                            cancelled = runs.count { r -> r.conclusion == "cancelled" },
                            recentFailures = runs.filter { r -> r.conclusion == "failure" }.take(5).map { r -> r.name },
                            slowestBuilds = runs.sortedByDescending { r -> r.runDurationMs }.take(5).map { r -> "${r.name} (${r.runDurationMs / 1000}s)" },
                            isLoading = false
                        )
                    }
                },
