...
interface RepositoryDao {
    ...
    @Query("DELETE FROM repositories WHERE ownerLogin = :owner AND name = :name")
    suspend fun deleteById(owner: String, name: String)
}
...
interface BranchDao {
    ...
    // PRD §23: Repository-scoped queries
    @Query("SELECT * FROM branches WHERE ownerLogin = :owner AND repoName = :repo")
    fun observeBranchesForRepo(owner: String, repo: String): Flow<List<BranchEntity>>

    @Query("SELECT * FROM branches WHERE repoId = :repoId")
    fun observeBranches(repoId: Long): Flow<List<BranchEntity>>

    @Query("DELETE FROM branches WHERE ownerLogin = :owner AND repoName = :repo")
    suspend fun clearForRepoScopes(owner: String, repo: String)
...
interface CommitDao {
    ...
    // PRD §23: Repository-scoped queries
    @Query("SELECT * FROM commits WHERE ownerLogin = :owner AND repoName = :repo ORDER BY date DESC")
    fun observeCommitsForRepo(owner: String, repo: String): Flow<List<CommitEntity>>

    @Query("SELECT * FROM commits WHERE repoId = :repoId ORDER BY date DESC")
    fun observeCommits(repoId: Long): Flow<List<CommitEntity>>

    @Query("DELETE FROM commits WHERE ownerLogin = :owner AND repoName = :repo")
    suspend fun clearForRepoScopes(owner: String, repo: String)
...
interface WorkflowDao {
    ...
    // PRD §23: Repository-scoped queries — use ownerLogin + repoName
    @Query("SELECT * FROM workflows WHERE ownerLogin = :owner AND repoName = :repo")
    fun observeWorkflowsForRepo(owner: String, repo: String): Flow<List<WorkflowEntity>>

    @Query("SELECT * FROM workflows WHERE repoId = :repoId")
    fun observeWorkflows(repoId: Long): Flow<List<WorkflowEntity>>

    @Query("DELETE FROM workflows WHERE ownerLogin = :owner AND repoName = :repo")
    suspend fun clearForRepoScopes(owner: String, repo: String)
...
interface WorkflowRunDao {
    ...
    // PRD §23: Repository-scoped queries
    @Query("SELECT * FROM workflow_runs WHERE ownerLogin = :owner AND repoName = :repo ORDER BY updatedAt DESC")
    fun observeRunsForRepo(owner: String, repo: String): Flow<List<WorkflowRunEntity>>
    ...
    @Query("SELECT * FROM workflow_runs WHERE id = :runId LIMIT 1")
    suspend fun getById(runId: Long): WorkflowRunEntity?
    ...
    @Query("DELETE FROM workflow_runs WHERE ownerLogin = :owner AND repoName = :repo")
    suspend fun clearForRepoScopes(owner: String, repo: String)
...
interface JobDao {
    ...
    @Query("DELETE FROM jobs WHERE runId IN (SELECT id FROM workflow_runs WHERE ownerLogin = :owner AND repoName = :repo)")
    suspend fun clearForRepoScopes(owner: String, repo: String)
}
...
interface ArtifactDao {
    ...
    @Query("DELETE FROM artifacts WHERE runId IN (SELECT id FROM workflow_runs WHERE ownerLogin = :owner AND repoName = :repo)")
    suspend fun clearForRepoScopes(owner: String, repo: String)
}
...
interface OperationDao {
    ...
    @Query("DELETE FROM operations WHERE ownerLogin = :owner AND repoName = :repo")
    suspend fun clearForRepoScopes(owner: String, repo: String)
}
