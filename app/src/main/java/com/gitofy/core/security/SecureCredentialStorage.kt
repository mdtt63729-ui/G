...
class SecureCredentialStorage @Inject constructor(
    ...
    fun saveAiKey(provider: String, apiKey: String) {
        encryptedPrefs.edit().putString("ai_key_$provider", apiKey).apply()
    }

    fun getAiKey(provider: String): String? = encryptedPrefs.getString("ai_key_$provider", null)

    fun hasAiKey(provider: String): Boolean = encryptedPrefs.contains("ai_key_$provider")
    ...
    private val providerInstancePrefs =
        context.getSharedPreferences("gitofy_provider_instances", Context.MODE_PRIVATE)
            ...
            for (i in 0 until json.length()) {
                ...
                list.add(
                    com.gitofy.ai.provider.registry.ProviderInstance(
                        ...
                        selectedModel = obj.optString("selectedModel", null)
