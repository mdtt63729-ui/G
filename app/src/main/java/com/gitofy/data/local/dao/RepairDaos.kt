...
interface GitoRepairJobDao {
    ...
    @Query("DELETE FROM gito_repair_jobs WHERE ownerLogin = :owner AND repoName = :repo")
...
interface GitoRepairAttemptDao {
    ...
    @Query("DELETE FROM gito_repair_attempts WHERE repairId NOT IN (SELECT repairId FROM gito_repair_jobs)")
    suspend fun clearOrphaned()
}
