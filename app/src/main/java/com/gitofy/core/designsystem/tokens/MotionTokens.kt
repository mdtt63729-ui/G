...
object MotionTokens {
    ...
    /**
     * PRD §22: slide + spring/bounce for the scroll-aware Create FAB.
     * Deliberately used only for this single, small, isolated element —
     * a noticeable-but-not-excessive overshoot reads as a nice accent
     * without introducing jitter on a large moving surface.
     */
    val FabBounceSpring: SpringSpec<Float> = spring(
        dampingRatio = Spring.DampingRatioMediumBouncy,
        stiffness = Spring.StiffnessMedium
    )

    val FabBounceSpringInt: SpringSpec<androidx.compose.ui.unit.IntOffset> = spring(
        dampingRatio = Spring.DampingRatioMediumBouncy,
        stiffness = Spring.StiffnessMedium
    )
}
